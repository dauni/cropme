  ...
  function createContext() {
    ...
    let up = function () {
      ...
      let scale = self.properties.scale,
        ...
        cx,
        cy
      let ox = parseFloat(origin[0])
      let oy = parseFloat(origin[1])
      ...
      console.log((nx-x)*scale,(ny-y));
      // console.log((nx-x)*(1-scale),(ny-y)*(1-scale));
    ...
    //   cy = ny - y
      ...
      let disx = self.properties.ox - self.properties.x
      let disy = self.properties.oy - self.properties.y
      let a = cx * Math.cos(angle) - cy * Math.sin(angle)
      let b = cx * Math.sin(angle) + cy * Math.cos(angle)
