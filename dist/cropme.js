...
(function (global, factory) {
  ...
  function createContext() {
    ...
    var up = function up() {
      ...
      var deg = self.properties.deg;
      self.properties.deg = 0;
      self.properties.image.style.transform = transform.call(self);
      ...
      var oy = parseFloat(origin[1]); // let deg = -parseInt(self.properties.deg)

      cx = left / scale;
      cy = top / scale;
      self.properties.deg = deg;
      var angle = -parseInt(self.properties.deg) * Math.PI / 180;
      console.log(cx, cy); // if (true) {

      var old_originx = self.properties.rotate_originx;
      var old_originy = self.properties.rotate_originy;
      var nx = ox - cx;
      var ny = oy - cy;
      var x = nx * Math.cos(angle) - ny * Math.sin(angle);
      var y = nx * Math.sin(angle) + ny * Math.cos(angle);
      console.log(x * scale, y);
      console.log((nx - x) * scale, ny - y); // console.log((nx-x)*(1-scale),(ny-y)*(1-scale));
      //  cx = nx - x
      //   cy = ny - y
      ...
      // } else {
      // }
      // console.log((cx - ox) / (1 - scale), (cy - oy) / (1 - scale));

      var disx = self.properties.ox - self.properties.x;
      var disy = self.properties.oy - self.properties.y;
      self.properties.x = self.properties.x - (cx - ox) * (1 - scale);
      self.properties.y = self.properties.y - (cy - oy) * (1 - scale);
      self.properties.image.style.transformOrigin = transformOrigin.call(self, cx + x * scale, cy + y * scale);
      self.properties.image.style.transform = transform.call(self);
    };
    ...
    _createClass(Cropme, [{
      ...
      value: function position() {
        return {
          ...
          scale: parseFloat(this.properties.scale.toFixed(6)),
